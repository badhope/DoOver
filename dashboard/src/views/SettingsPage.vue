                ...
                <div class="provider-detail">
                  <div class="provider-config-panel">
                    ...
                    <div class="provider-model-panel">
                        ...
                        <div v-if="providerModelOptions.length" class="provider-models-list">
                          ...
                          >
                            ...
                            <span class="provider-model-row-actions">
                              <button
                                type="button"
                                class="provider-model-test-btn"
                                :disabled="providerModelTestingName === option.name"
                                :title="providerModelTestingName === option.name ? '测试中...' : '测试模型连接'"
                                @click.stop.prevent="handleTestProviderModel(option.name)"
                              >
                                <span
                                  v-if="providerModelTestingName === option.name"
                                  class="provider-model-loading-spinner"
                                  aria-hidden="true"
                                ></span>
                                <svg
                                  v-else
                                  class="provider-model-test-icon"
                                  viewBox="0 0 24 24"
                                  aria-hidden="true"
                                >
                                  <path :d="mdiConnection"></path>
                                </svg>
                              </button>
                      ...
                      <div
                        v-if="providerModelTestResult"
                        class="provider-test-result"
                        :class="{ error: !providerModelTestResult.success }"
                      >
                        <div class="provider-test-result-head">
                          <span class="provider-test-result-label">测试结果</span>
                          <span class="provider-test-result-model">
                            {{ providerModelTestResult.provider }} / {{ providerModelTestResult.model }}
                          </span>
                        </div>
                        <p class="provider-test-result-text">{{ providerModelTestResult.text }}</p>
                      </div>
...
function resetProviderEditor() {
  ...
  providerModelTestingName.value = "";
  providerModelTestResult.value = null;
  providerModelOptions.value = [];
}
...
async function handleTestProviderModel(modelName: string) {
  const normalizedModelName = String(modelName || "").trim();
  if (!normalizedModelName || providerModelTestingName.value) return;

  providerError.value = "";
  providerMessage.value = "";
  providerModelTestingName.value = normalizedModelName;
  providerModelTestResult.value = null;

  try {
    const data = await requestJson<TestModelResponse>("/test_model", {
      method: "POST",
      body: JSON.stringify({
        provider: providerForm.value.provider || undefined,
        type: providerForm.value.type || defaultProviderType.value,
    ...
    providerModelTestResult.value = {
      provider: data.provider || "临时配置",
      model: data.model,
      text: data.result,
      success: true,
    };
    providerMessage.value = `模型 ${normalizedModelName} 测试成功`;
  } catch (error: unknown) {
    const message = error instanceof Error ? error.message : "模型测试失败";
    providerModelTestResult.value = {
      provider: providerForm.value.provider || "临时配置",
      model: normalizedModelName,
      text: message,
      success: false,
    };
...
.provider-model-test-btn:hover:not(:disabled) {
  color: var(--accent);
  background: transparent;
}

.provider-model-test-btn:disabled {
  opacity: 0.92;
  cursor: wait;
}

.provider-model-test-btn:focus,
.provider-model-test-btn:focus-visible {
  outline: none;
  box-shadow: none;
}

.provider-model-test-icon {
  width: 22px;
  height: 22px;
  fill: currentColor;
}

.provider-model-loading-spinner {
  width: 26px;
  height: 26px;
  border-radius: 50%;
  background: conic-gradient(
    from -90deg,
    rgba(24, 24, 27, 0.18) 0deg,
    rgba(24, 24, 27, 0.32) 90deg,
    rgba(24, 24, 27, 0.58) 180deg,
...
.provider-test-result {
  ...
  background: rgba(31, 138, 83, 0.08);
}
